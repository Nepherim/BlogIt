...
jQuery(document).ready(function($) {
	...
	$(document).on("click", BlogIt.pm['skin-classes']['comment'] + '.blogit-admin', function(e) {
	...
	$(BlogIt.pm['skin-classes']['blog-form'] + ' form :input:not(:submit)').on('change', //if any field (not a submit button) changes...
		function() {
			$(window).on('beforeunload', function() {
				return BlogIt.fn.xl('You have unsaved changes.');
			});
		});
});
...
BlogIt.fn = function($) {
	...
	$.ajaxSetup({
		...
		error: function(request, error) {
			BlogIt.fn.showMsg({
				result: 'error',
				msg: (
					(error == 'parsererror' ? 'Parsing JSON request failed.' : (error == 'timeout' ? 'Request timeout.' : 'Error: ' + error + "\n" + request.readyState + "\nresponseText: " + request.responseText))
				)
			});
		}
	});
	...
	function dialogShow(txt, yes, no, w, ajax) {
		...
		var prompt = new jBox('Confirm', {
			...
			width: w,
			minWidth: w,
			maxWidth: w //needed to override jbox default
		}).open();
	}
	...
	return {
		...
		commentAdminCheckbox: function(src, action, opt) { //src [flip|show|hide]
			if (action == 'flip') {
				$('input[name="bi_CommentID[]"]', src).prop("checked", function() {
			...
			if (url.rc > 0) {
				if (action == 'delete') {
					...
					BlogIt.fn.ajax({
						url: url.url,
						success: function(data) {
							if (data.result == 'error') BlogIt.fn.showMsg(data);
							flipCommentStatus(url.e, action);
						}
					});
				}
			}
		...
		addAutocomplete: function() {
			...
			$(document).on("blur", '#entrytags', function(e) {
				$this = $(this);
		...
		xl: function(t) {
			return ((BlogIt.xl[t] ? $('<div>' + BlogIt.xl[t] + '</div>').html() : t));
		},
